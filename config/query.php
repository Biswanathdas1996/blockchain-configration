...
function select($table, $condition = [])
{
    ...
    if (isset($condition['order'])) {
        ...
        foreach ($order as $names => $vals) {
            $orders =
                mysqli_real_escape_string($conn, $names) .
                ' ' .
                mysqli_real_escape_string($conn, $vals);
        }
    } else {
        $orders = 'id asc';
    }
    ...
    if (isset($conditions)) {
        ...
        foreach ($conditions as $name => $val) {
            $columns[] =
                "$name = '" . mysqli_real_escape_string($conn, $val) . "'";
        }
        $sql =
        ...
        while ($row = mysqli_fetch_assoc($retval)) {
            ...
            if (isset($condition['join'])) {
                foreach ($condition['join'] as $keys => $values) {
                    ...
                    $result_join = mysqli_query(
                        $linkp,
                        "SHOW COLUMNS FROM $join_table"
                    );
                    if (mysqli_num_rows($result_join) > 0) {
                        $form_data_join = [];
                        while (
                            $row_join_table = mysqli_fetch_assoc($result_join)
                        ) {
                            $form_data_join[] = $row_join_table['Field'];
                        }
                    }
                    $join_table_id = mysqli_real_escape_string(
                        $conn,
                        $row[$values]
                    );
            ...
            if (isset($condition['join_many'])) {
                foreach ($condition['join_many'] as $keys => $values) {
                    ...
                    if (isset($condition['join_many_conditions'])) {
                        ...
                        $sql_join_many =
                            "SELECT * FROM `$join_table`  WHERE `$foregin_key` = '$join_id' AND " .
                            implode(' AND ', $join_columns) .
                            '';
                    } else {
                    ...
                    if (mysqli_num_rows($retval_joint_many) > 0) {
                        $many_data = [];
                        while (
                            $row_join_many = mysqli_fetch_assoc(
                                $retval_joint_many
                            )
                        ) {
                            $data_join_many = [];
                            foreach ($form_data_join as $values) {
                                $data_join_many[$values] =
                                    $row_join_many[$values];
                            }
                            if (
                                (int) $join_test_id ===
                                (int) $data_join_many[$foregin_key]
                            ) {
    ...
    if (mysqli_query($conn, $sql)) {
        ...
        if (!empty($conditions['file'])) {
            foreach ($conditions['file'] as $key => $up_img) {
                ...
                $ext = pathinfo(
                    $_FILES[$up_img['name']]['name'],
                    PATHINFO_EXTENSION
                );

                $get_current_file = select($table, [
                    'conditions' => [
                        'id' => $id,
                    ],
                ]);
